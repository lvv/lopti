...
#define  CONVERT_GSl_H
	...
	#include <lvv/array.h>
			...
			template <typename T, int N, int B>       lvv::array<T,N,B>&
	 operator<<=  (lvv::array<T,N,B>& A, const gsl_vector* gV)  {	// operator= should be member, so we are using operator<<
	 ...
	 operator<<=  (gsl_vector* gV, lvv::array<T,N,B>& A)  {
		assert(A.size()==gV->size);  assert(A.ibegin()==0);  
