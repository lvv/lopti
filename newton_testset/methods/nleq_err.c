/*
    NLEQ_ERR - ERRor-based Damped Newton algorithm
 ...
 *    Parameters description
      ...
      extern void nleq_err(struct NLEQ_FUN funs, int n, double *x,
                           struct NLEQ_OPT *opt, 
      ...
      will be approximately computed by an internal function of the nleq_err
      package.
      ...
      contain input options to nleq_err.
      ...
      header file. It must be set either to a NULL pointer, stderr, stdout,
      or to another file pointer which has been initialized by a fopen call.
      If it is set to NULL, a file named "nleq_err.data" will be opened by a
      ...
      identification number of the calling code (3 for NLEQ_ERR), the norm
      ...
      are set output info of nleq_err.
      ...
      info->rcode is set to the return-code of nleq_err. A return-code 0
      ...
      void fun(int *n, double *x, double *f, int *fail);
        ...
        int    *fail  output fun evaluation-failure indicator.
                      ...
                      On output: Indicates failure of fun evaluation, 
                                 if having a value <= 2.
                      If <0 or >2: nleq_err will be terminated with
      ...
      void jac(int *n, int *ldjac, double *x, double *dfdx, int *fail);
        ...
        int    *fail  output fun evaluation-failure indicator.
                      ...
                      and causes termination of nleq_err, f set to a nonzero
                      value on output.
      ...
      Note: The calling interfaces of the user routines fun and jac has
            ...
            or you must compile the nleq_err package for use with Fortran
      ...
      -989 Default data-output file could not be opened via fopen call.
       -99 NULL pointer obtained from funs.fun field - the problem function
         ...
         1 Singular Jacobian matrix (detected by routine nleq_linfact),
           nleq_err cannot proceed the iteration.
...
int nleqerr_initscale(int n, double **scale, struct NLEQ_OPT *opt);
void nleqerr_rescale(int n, double *scale, double *x, double *xa,
                     double *xthresh);
void nleqerr_monitor(int k, int n, double normdx, double normf,
                     double lambda, char indicator);
...
extern void nleq_err(struct NLEQ_FUN fun, int n, double *x,
                  struct NLEQ_OPT *opt, struct NLEQ_INFO *info)
{  int inc_one=1;
   double one=1.0, minus_one=-1.0;
   double xtol=opt->tol;
   double lambda, lambda_new, mue, normfk, normfkp1, normdxkm1, 
          normdxbar, reduction_factor, normdx, theta, s;
   double lambda_min;
   int i, j, k=0, fail=0, nrhs=1, ldjac,
            max_iter=opt->maxiter;
   LOGICAL     qnerr_iter=False,
   ...
   if ( !DATA && DATALEVEL>0 )
     { DATA=fopen("nleq_err.data","w");
       if (!DATA && ERRORLEVEL>0)
         { fprintf(ERROR,"\n fopen of file nleq_err.data failed\n");
           RCODE=-989; return;
         };
     };
   opt->errorfile   = ERROR;
   ...
   RCODE = nleq_parcheck_and_print(n,opt,fun,3);
   if ( RCODE !=0 ) 
   ...
   RCODE = nleq_fwalloc(n,&dxbar,"dxbar");     if ( RCODE !=0 ) return;
   RCODE = nleq_fwalloc(n,&xkp1,"xkp1");       if ( RCODE !=0 ) return;
   RCODE = nleq_fwalloc(n,&fxk,"fxk");         if ( RCODE !=0 ) return;
   RCODE = nleq_fwalloc(n,&xthresh,"xthresh"); if ( RCODE !=0 ) return;
   ...
   xscale = opt->scale;
   if ( xscale == NULL ) 
     {RCODE=nleqerr_initscale(n,&xscale,opt); if (RCODE !=0) goto errorexit;
      scale_allocated = 1;};
   if ( MONITORLEVEL > 1 ) 
   ...
   do 
     { nleqerr_rescale(n,xscale,x,w,xthresh);
       if ( k>0 )
         /* recompute norms after rescaling */
         { normdxkm1 = nleq_scaled_norm2(n,dx,xscale);
           normdxbar = nleq_scaled_norm2(n,dxbar,xscale);
         };
       if (*jc != NULL) 
       ...
       for (i=0;i<n;i++) dx[i]=fxk[i];
       fail = nleq_linsol(n,dx,opt);
       if ( fail != 0 ) { RCODE=81; goto errorexit; };
       nleq_descale(n,dx,dx,xscale);
       normdx = nleq_scaled_norm2(n,dx,xscale);
       if ( MONITORLEVEL > 1 ) 
         nleqerr_monitor(k,n,normdx,normfk,lambda,' ');
       ...
       if ( k>0 )
         { for (i=0;i<n;i++) w[i] = dxbar[i]-dx[i];
           s = nleq_scaled_norm2(n,w,xscale);
       ...
       normfkp1 = nleq_norm2(n,fxk);
       for (i=0;i<n;i++) dxbar[i]=fxk[i];
       fail = nleq_linsol(n,dxbar,opt);
       if ( fail != 0 ) { RCODE=81; goto errorexit; };
       nleq_descale(n,dxbar,dxbar,xscale);
       normdxbar = nleq_scaled_norm2(n,dxbar,xscale);
       if ( MONITORLEVEL > 1 ) 
         nleqerr_monitor(k,n,normdxbar,normfkp1,lambda,'*');
       theta = normdxbar/normdx;
       s = 1.0-lambda;
       for (i=0;i<n;i++) w[i] = dxbar[i]-s*dx[i];
       mue = (0.5*normdx*lambda*lambda) / nleq_scaled_norm2(n,w,xscale);
       ...
       if ( lambda==1.0 && lambda_new==1.0 )
         {
           if ( normdxbar <= xtol )
             { for(i=0;i<n;i++) x[i] += dxbar[i];  RCODE=0;  break; };
       ...
       k++;
       normfk = normfkp1;
       /* perform qnerr steps, if choosen */
       if (qnerr_iter)
         { info->iter=k;
           ...
           info->dx = dx;
           info->normdx = normdx;
           qnerr(fun,n,x,opt,info);
           nfcn += info->nofunevals;
           k = info->iter;
           normdx=info->precision;
           opt->errorlevel = error_level;
           ERRORLEVEL      = error_level;
           /* if QNERR failed, try to continue NLEQ_ERR */
           if ( RCODE != 0 ) {RCODE = 2; qnerr_iter=False; };
         }
     }
   ...
   if (io_allocated) {free(nleq_ioctl); nleq_ioctl=NULL;};
   free(data);
   free(dx); free(xkp1); if (qnerr_iter) free(fxk);
   free(w);
...
void nleqerr_monitor(int k, int n, double normdx, double normf,
                     double lambda, char indicator)
{  fprintf(MONITOR," %4i  %1c  %12e  %12e  %7f\n",
                   k,indicator,normdx,normf,lambda);
}
